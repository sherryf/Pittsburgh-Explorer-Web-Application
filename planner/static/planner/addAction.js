...
function updateList(response){
    ...
    if(elements.children.length == 0){
        ...
        var startLocation = document.getElementById("addr-0").innerHTML.split(',');
        var lastLat = startLocation[0];
        var lastLng = startLocation[0];
        var hrefgoogle = "https://www.google.com/maps?saddr=" + lastLat +"," + lastLng + "&daddr="+curLat+"," + curLng
        var latestendTime = document.getElementById("endtime-0").innerHTML;
        var startTime = add(latestendTime, zhongjianzhi);

        $(elements).append(
            "<div id='1' draggable ='true'>"+
            "<p class='hidden' id='addr-1'>" +addItem.lat +"," +addItem.lng+ "</p>" +
            "<p class='hidden' id= 'time-end-1' >" +"11:59" + "</p>" +
            ...
            "<div class='time' id='time-1'>" +startTime +"</div>" +
            "</div>" +
            ...
            "<div class='duration'>" +
            "<div class='durationTime' id='time-duration-1' >" + timeDuration +"h</div>" + ////
            "</div>" +
        ...
        var actualNum = elements.lastElementChild.children[0].parentNode.innerHTML;
        console.log(elements.lastElementChild);
        var indexdash = actualNum.indexOf('-');
        var numLen = parseInt(actualNum.substring(indexdash+1, indexdash+2));
        var childNum = numLen + 1;
...
function add_alert(itemid) {
    ...
    var itemCount = elements.children.length;
    console.log(itemCount);

    if(itemCount > 5){
        alert('No more than 5 events.');
        return;
    }
